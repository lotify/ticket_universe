...
class UniverseTest(unittest.TestCase):
    ...
    def test_universe_size_equals_size_cartesian_product_size(self):
        ...
        self.assertEqual(8, len(tickets), 'A binary universe with three positions should be length 2 ** 3 == 8')
        self.assertEqual(len(universe), len(tickets), 'Universe length should equal resulting ticket list length')
    ...
    def test_limited_universe(self):
        ticket_limit = 3
        universe = Universe([BinaryPosition(), BinaryPosition(), BinaryPosition()], limit=ticket_limit)
        tickets = [ticket for ticket in universe]
        self.assertEqual(ticket_limit, len(tickets), 'The universe should obey the ticket limit')

    def test_offset_universe(self):
        offset = 2
        universe = Universe([BinaryPosition(), BinaryPosition(), BinaryPosition()], offset=offset)
        tickets = [ticket for ticket in universe]
        self.assertEqual(6, len(tickets))

    def test_offset_limit_universe(self):
        universe = Universe([BinaryPosition(), BinaryPosition(), BinaryPosition()], limit=4, offset=2)
        tickets = [ticket for ticket in universe]
